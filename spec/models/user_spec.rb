...
describe User do
  before do 
    @user = User.create(name: "Testuser", email: "user@server.com", password: "foobar", password_confirmation: "foobar")
  end
  subject {@user}
  ...
  describe "when password is not presenet" do
  ...
  describe "when password doesen match confirmation" do
    before {@user.password = "foo"; @user.password_confirmation = "bar"}
    it{should_not be_valid}
  end
  ...
  describe "when email is not unique" do
    before {@user2 = User.create(name: "Foo", email: "USER@server.com")}
